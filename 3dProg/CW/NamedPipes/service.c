...
void WINAPI ServiceMain() {
    ...
    // Вызываем функцию, которая выполняет все необходимые инициализирующие
    // действия
    res = ServiceStart();
    if (res < 0) {
    ...
    // После завершения инициализации устанавливаем состояние работающего
    // сервиса
...
void WINAPI ServiceControl(DWORD dwControlCode) {
    ...
    case SERVICE_CONTROL_STOP: {
        ...
        // Выполняем остановку сервиса, вызывая функцию, которая выполняет все
