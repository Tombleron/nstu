...
#define strings_count 5 // Указываем колтчество строк
...
void copy(char *str1, char *str2) {
	while (*str1++ = *str2++); // Стоило бы проверять соответствие длин строк,
				   // но так как они одинаковой длины, то можно опустить
}
...
int get_num_count(char *str) {
	...
	while (*str != '\0') {		 	// Пока не наткнемся на \0 идем по строке
		if (48 <= *str && *str <= 57) { // и увеличиваем счетчик, если код сивола соответствует цифре
		       count++;
		}
		str++;
	}
...
int input(char *str, int maxlen) {
	...
	scanf("%s", inp_str);
	j = str_len(inp_str);
	while (j >= maxlen) {					// Если длина введенной строки больше максимальной, то просим ввести еще раз
		printf("Str length must be <= %d\n", maxlen);
...
void sort(char strs[strings_count][strings_len], int *nums, int first, int last) {
	if (first < last) {
		...
		do {
			...
			if (left <= right) {
				string_replace(strs[left], strs[right]); // Так как сортируем мы на самом деле массив с количеством цифр,
				num_replace(nums, left, right);		 // то для того, чтобы отсортировать массив строк, достаточно
				left++;					 // вносить такие же изменения, так как соотносящиеся элементы 
				right--;				 // имеют одинаковый индекс
			}
...
void output(char *str, int num) {
	printf("Строка: %s\n", str);
	if (num >= 2) {					// Вывод может изменится в зависимости от количества цифр в строке
		char *tmp = str;			// В случае, если цифр больше 1, то мы можем вывести вторую цифру
		int cnt = 0;
		char snd_num;
		while (cnt != 2) {
			if (*tmp >= 48 && *tmp <= 57) { // Тут можно спокойно прибавлять в указателю без проверки на конец строки,
				snd_num = *tmp;		// потому что мы точно знаем, что в строке как минимум две цифры
				cnt++;
			}
			tmp++;
		}					
		printf("Количество цифр: %d\nВторая цифра: %c\n\n", num, snd_num);
	} else {					// Если же в строке меньше 2 цифр, то выводится соответствующее сообщение
